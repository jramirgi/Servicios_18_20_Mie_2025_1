    ...
    public class clsCliente
    {
        ...
        public IQueryable ClientesConTelefonos()
        {
            return from C in dbSuper.Set<CLIEnte>()
                   ...
                   on C.Documento equals T.Documento into TeC
                   from x in TeC.DefaultIfEmpty()
                   orderby C.PrimerApellido, C.SegundoApellido, C.Nombre
                   group TeC by new { C.Documento, C.Nombre, C.PrimerApellido, C.SegundoApellido, C.FechaNacimiento, C.Email, C.Direccion }
                   into g
                   select new
                   {
